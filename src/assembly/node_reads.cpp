...
ReadId Node::getTerminalRead( bool drxn )
{
    ReadId id;
    int32_t limit = ends_[!drxn];
    for ( auto& read : reads_ ) if ( !read.second.redundant && ( drxn ? limit < read.second[1] : read.second[0] < limit ) )
    {
        limit = read.second[drxn];
        id = read.first;
        if ( limit == ends_[drxn] ) return id;
    }
    return id;
}
