...
//    Nodes( Node* base, Node* avoid, NodeOffsets &offs, bool drxn, bool inclBase );
    ...
    void operator += ( vector<Node*> &adds );
    void operator -= ( Node* node );
    void operator -= ( Nodes &rhs );
    bool add( Node* node );
...
//    bool fill( Node* node, NodeOffsets &offs, bool drxn );
    ...
    vector<Node*> meet( Node* node );
    void meet( Node* node,  Nodes& pathed, vector<Node*>& met );
...
struct NodeRoll
{
    ...
    bool cull( string s );
    bool find( Node* node );
    void flip( Nodes& flips );
    void erase( Node* node );
    void erase( Node* node, int &i );
    void drop( Node* node );
    bool empty();
    NodeRoll getGraph( int drxn );
    bool getLine( string& s, vector<Node*>& line );
    static NodeRoll next( Node* node, bool drxn );
    void remove( Node* node );
    void print( bool inclBad );
    void print( string fn, int32_t i, int32_t j );
    void print( ofstream& ofs, Node* node, Nodes used[2], int32_t limit );
    bool purify( string s );
    void purify( vector<Node*>& base, vector<Node*>& tar, bool drxn );
    int size();
    bool solidify( string s );
    bool solidify( vector<Node*>& line );
    bool solidify( string& s, vector<Node*>& line );
    void summarise();
    void test( bool loop=false );
    void updateBad();
    void verify( int drxn );
    vector<Node*> nodes;
};
...
struct NodeCounts
{
    void countOverlaps( Node* node, int ol, int minOl, bool drxn, bool inclNode=false );
    void countReads( Node* node, bool drxn );
    int32_t* get( Node* node );
    unordered_map<Node*, int32_t> map;
};
