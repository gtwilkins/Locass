...
{
    int count = 0;
    if ( !fork->bad_ ) for ( Edge& e : fork->edges_[drxn] ) if ( !e.leap && !e.node->bad_ ) count++;
    if ( count < 2 ) return false;
    
    Nodes fwd( fork, params.readLen*2, drxn, false );
    for ( Edge& e : fork->edges_[drxn] ) if ( !e.node->bad_ && e.node->edges_[!drxn].size() < 2 )
    {
...
void Bubble::set( Node* node, vector<Node*>& path, int32_t dist, Nodes& fwd, bool drxn )
{
    ...
    if ( find( path.begin(), path.end(), node ) != path.end() ) return;
