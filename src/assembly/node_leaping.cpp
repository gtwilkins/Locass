...
//    {
//        for ( Node* b : Nodesx( ( drxn ? path[0] : path.back() ), params.maxPeMean - ( ends[1] - ends[0] ), !drxn, true, false ).nodes )
//        {
//            b->branch_ = !b->bad_;
//        }
//    }
...
//    
//    Nodesx island, ignore;
//    for ( Node* n : leapTarget( true, drxn ).nodes ) ignore.fillBad( n, true, !drxn );
//    for ( Node* b : Nodesx::isBranch( this ).nodes ) for ( Node* n : nodes.nodes ) if ( b->verify( n, b->pe_[drxn], 0, drxn ) && !ignore.find( n )  ) island.add( n );
//    
//    nodes.test();
//    for ( int lept = 1; lept-- > 0; )
//    {
//        for ( Node* b : Nodesx::isBranch( this ).nodes )
//        {
//            if ( lept ) break;
...
//                    Nodesx seeds = Nodesx::isBad( node, true, drxn );
...
//    Nodesx tar = leapTarget( true, drxn );
...
//    }
//    assert( maxOl );
//    Nodesx branches = Nodesx::isBranch( this ), tested;
...
//        {
//            NodeRoll ext;
//            Nodesx tar = leapTarget( true, drxn ), extable, fwd = Nodesx::isBad( seed, true, d );
//            NodeScores scores;
...
//}
//
//NodeIntList Node::leapScore( NodeScores& scores, Nodesx& allowed, bool drxn )
//{
//    NodeIntList hits;
//    for ( auto & ns : scores.scores )
//    {
//        int score = ns.second;
//        for ( Node* b : Nodesx::inSet( ns.first, allowed, drxn, false ).nodes ) score += scores.get( b );
...
//}
//
//Nodesx Node::leapTarget( bool resetGood, bool drxn )
//{
//    Nodesx tar( this, params.maxPeMean, !drxn, true );
//    tar.dumpBad( true );
//    for ( Edge &e : edges_[drxn] ) e.node->leapTarget( tar, 10, resetGood, drxn );
//    return tar;
//}
//
//void Node::leapTarget( Nodesx& tar, int extReadMax, bool resetGood, bool drxn )
//{
