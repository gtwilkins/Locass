...
{
    Leap l( bwt, nodes, offs, node->ends_[drxn], drxn );
    if ( node->ends_[0] == -10836 )
    {
        for ( auto& read : node->reads_ ) if ( params.isReadPe( read.first ) ) 
        {
            ReadId id = read.first;
            if ( params.setPairId( id, drxn ) && node->reads_.find( id ) == node->reads_.end() )
            {
                cout << ">" << id << endl;
                cout << string( abs( node->ends_[drxn] - read.second[drxn] ), '-') << bwt.getSequence( id ) << endl;
            }
        }
        int x = 0;
    }
