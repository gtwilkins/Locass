...
bool PathCross::claim( vector<NodePath*>& paths, NodeRoll& nodes )
{
    if ( !crossed_ )
    {
        ...
        int score[2][2]{0};
        for ( int d : { 0, 1 } ) if ( ( score[d][0] = branch_[d][0]->pairs_[1]->score_ ) < ( score[d][1] = branch_[0][0]->pairs_[0]->score_ ) * 4 + 4 ) return false;
        cout << "Deforked: " << score[0][0] << " | " << score[0][1] << "   " << score[1][0] << " | " << score[1][1] << endl;
    ...
    for ( CrossPair* cp : paired ) if ( matches[0][ cp->branch[0] ] < 2 && matches[1][ cp->branch[1] ] < 2 && separate( cp->branch, paths, nodes ) )
    {
        cout << "Decrossed branch   " << paired.size() << " pairs:   " << cp->score_ << "   >   " << cp->miss_ << "   Cover: " << cross_[0]->cover_ << endl;
        matched = true;
    }
    ...
    {
        return false;
        cout << "Decrossed fork   " << best << endl;
        separate( branches, paths, nodes );
    }
...
//            unordered_set<NodePath*> pathed( cross_.begin(), cross_.end() );
            unordered_map<NodePath*, int32_t> pathed;
            for ( NodePath* np : cross_ ) pathed.insert( make_pair( np, 0 ) );
...
bool PathCross::reach( NodePath* np, unordered_map<NodePath*, int32_t>& pathed, vector< pair<NodePath*, int32_t> >& tars, int32_t diff, int32_t cutoff )
{
    for ( pair<NodePath*, int32_t>& t : tars ) if ( t.first == np && t.second - diff < cutoff ) return true;
    if ( cutoff < np->ends_[1] - cutoff ) return false;
    
    auto ins = pathed.insert( make_pair( np, diff ) );
    if ( !ins.second )
    {
        if ( diff - 50 < ins.first->second ) return false;
        ins.first->second = max( diff, ins.first->second );
    }
...
bool PathCross::resolve( vector<NodePath*>& paths, NodeRoll& nodes )
{
    ...
    for ( int again = 1; again-- > 0; )
    {
        ...
        int counts = 0;
        for ( PathCross* pc : crosses ) if ( pc->claim( paths, nodes ) ) counts++;
...
{
    CrossBranch* a = paired[0],* b = paired[1];
