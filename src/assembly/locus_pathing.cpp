...
bool Locus::plotPath( Path &path, bool drxn )
{
    ...
    vector<Allele> initAlleles = path.alleles;
    ...
    while ( best.branch )
    {
        ...
        if ( !plotPathConverge( pv, best.branch, path, drxn ) )
        {
            path.spans = initSpans;
            path.alleles = initAlleles;
            return false;
        }
        
        // Update current multiplicity state and update spans if changing state
        plotPathUpdateMultiplicity( best, last, path, notReliableNodes, drxn );
    ...
    // Set furthest fork
    if ( !plotPathSetForks( pv, path, goodSet, notReliableNodes, drxn ) )
    {
        path.spans = initSpans;
        path.alleles = initAlleles;
        return false;
...
{
    if ( !best.valid && !pv.invalid )
    {
        pv.invalid = best.branch;
        pv.invalidCoord = best.branch->ends_[!pv.drxn] + ( pv.drxn ? params.maxMpMean : -params.maxMpMean );
    }
    ...
    if ( score < params.branchMinHits && best.branch->drxn_ != 2 && abs( best.branch->ends_[!pv.drxn] - reliable_[!pv.drxn] ) > params.maxPeMean )
    {
        float ratio = score / params.branchMinHits;
        int32_t invalidCoord = pv.drxn ? max( best.branch->ends_[1], +params.maxPeMean ) + max( params.maxMpMean * ratio, (float)params.maxPeMean )
                                       : min( best.branch->ends_[0], -params.maxPeMean ) - max( params.maxMpMean * ratio, (float)params.maxPeMean );
        pv.invalidCoord = !pv.invalid ? invalidCoord 
                                      : ( pv.drxn ? min( pv.invalidCoord, invalidCoord ) 
                                                  : max( pv.invalidCoord, invalidCoord ) );
        pv.invalid = pv.invalid ? pv.invalid : best.branch;
    }
    
    // Check reliability and check for misassembly
    if ( best.branch->isReliable( false ) )
    {
        if ( abs( best.branch->ends_[!pv.drxn] - reliable_[!pv.drxn] ) > params.maxPeMean
                && best.branch->isMisassembled( bwt_, nodes_[pv.drxn], validLimits_, pv.drxn )
                && best.branch->drxn_ != 2 )
        {
            int32_t invalidCoord = pv.drxn ? best.branch->ends_[0] + params.maxPeMean
                                           : best.branch->ends_[1] - params.maxPeMean;
            pv.invalidCoord = !pv.invalid ? invalidCoord 
                                      : ( pv.drxn ? min( pv.invalidCoord, invalidCoord ) 
                                                  : max( pv.invalidCoord, invalidCoord ) );
            pv.invalid = pv.invalid ? pv.invalid : best.branch;
        ...
        {
            reliable_[pv.drxn] = pv.drxn ? max( reliable_[1], best.branch->ends_[1] ) 
                                         : min( reliable_[0], best.branch->ends_[0] );
        }
    }
    ...
    {
        path.divergent.push_back( branch );
    }

    path.path.push_back( best.branch );
    
    return !( pv.invalid && finalise_ );
}
...
bool Locus::plotPathConverge( PathVars &pv, PathBranch &convBranch, Node* convEnd, BranchList &newDiv, NodeSet &convSet, Path &path, bool drxn )
{
    ...
    plotPathConvergeScore( pv, bgnCoord, convEnd, scores[0], drxn );
    plotPathConvergeScore( pv, bgnCoord, convEnd, scores[1], drxn );
...
bool Locus::plotPathConvergeCompare( PathScore* scores, Path &path, bool drxn )
{
    ...
    bool notDiploid = coCover > params.cover * ( 1.1 + ( min( float(0.4), ( combLen / float( params.readLen ) ) / float(20) ) ) );
    
    // Is at least one not haploid?
    if ( scores[1].notHaploid || scores[0].notHaploid || notDiploid )
    {
        for ( int i : { 0, 1 } )
        {
            if ( !badFar[i] 
...
void Locus::plotPathConvergeScore( PathVars &pv, int32_t bgnCoord, Node* postFork, PathScore &score, bool drxn )
{
    score.len = -( score.path[0]->getBestOverlap( !drxn ) + score.path.back()->getOverlap( postFork, drxn ) ) / 2;
...
void Locus::plotPathGetFirst( PathVars &pv, PathBranch &best, Path &path, BranchList &branches, bool drxn )
{
    ...
    if ( best.branch )
    {
        ...
        path.isMulti = ( !path.spans.empty() && !path.spans.back().end ) || ( best.branch && best.branch->isMultiple() );
        if ( path.isMulti && ( path.spans.empty() || path.spans.back().end ) )
        {
...
void Locus::plotPathSetEnds( PathVars &pv, PathBranch &best, PathBranch &last, Path &path, bool drxn )
{
    ...
    assert( sideNodes_[drxn].empty() || desperation_[drxn] >= 3 || !completed_[drxn] || finished_[drxn] || pv.invalid );
}
...
bool Locus::plotPathSetForks( PathVars &pv, Path &path, NodeSet &goodSet, NodeList &notReliableNodes, bool drxn )
{
    ...
    if ( pv.invalid && pv.invalid != path.path.back()
            && ( drxn ? pv.invalidCoord < path.path.back()->ends_[1]
                      : path.path.back()->ends_[0] < pv.invalidCoord ) )
    {
        NodeSet delSet = pv.invalid->getDrxnNodes( drxn );
        deleteNodes( delSet, drxn );
        pv.invalid->stop_[drxn] = 5;
        return false;
    }

    for ( auto it = path.path.begin(); it != path.path.end(); it++ )
    {
        if ( ( !finalise_ && badBranches.find( *it ) != badBranches.end() ) || *it == pv.invalid ) 
        {
...
void Locus::plotPathTrimDivergent( PathVars &pv, Path &path,NodeSet &goodSet, NodeSet &delSet, bool drxn )
{
    ...
    for ( PathBranch &div : path.divergent )
    {
        ...
        else
        {
            ...
            if ( !path.path.empty() )
            {
                assert( div.fwdSet.find( path.path.back() ) == div.fwdSet.end() );
            }
        }
    }
}
...
void Locus::plotPathTrimPrep( PathVars &pv, Path &path, bool drxn )
{
    if ( !path.path.empty() )
    {
        path.path.back()->getDrxnNodes( pv.tSet, drxn );
    }
...
void Locus::plotPathUpdateMultiplicity( PathBranch &best, PathBranch &last, Path &path, NodeList &notReliableNodes, bool drxn )
{
    ...
    else if ( path.isMulti && best.branch->isReliable( true ) )
    {
        // Complete currently open span
        if ( !path.spans.empty() && !path.spans.back().end )
        {
            path.spans.back().setEnd( last.branch ? last.branch : best.branch, drxn );
        }
        path.isMulti = false;
    }
}
...
void Locus::plotPathUpdateSpans( PathBranch &best, Path &path, bool drxn )
{
    ...
    {
        bool spanned = span.spanned;
        span.spanned = span.spanned || ( span.end && best.valid );
        best.invalid = best.invalid ? : span.end && !span.spanned && best.branch->isFurther( params.getFurthestMpDist( span.begin->ends_[drxn], drxn ), drxn, drxn );
    }
}
...
{
    NodeSet pathSet( nodes_[2].begin(), nodes_[2].end() );
    for ( Node* fork : forkNodes_[drxn] )
    {
        fork->getDrxnNodes( pathSet, !drxn );
    }
    
    for ( Path &path : paths_[drxn] )
    {
        for ( auto it = path.spans.begin(); it != path.spans.end(); it++ )
        {
            if ( pathSet.find( (*it).begin ) == pathSet.end() )
            {
                path.spans.erase( it, path.spans.end() );
                break;
            }
            if ( pathSet.find( (*it).end ) == pathSet.end() )
            {
                (*it).end = NULL;
                (*it).spanned = false;
            }
        ...
        for ( auto it = path.spans.begin(); it != path.spans.end() && it+1 != path.spans.end(); it++ )
        {
            if ( !(*it).end )
            {
                (*it).end = (*it).begin;
