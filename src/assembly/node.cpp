...
Node::Node( Querier& bwt, string& seq, int graph )
: Node( seq )
{
    drxn_ = graph;
    vector<ReadId> ids;
...
Node::Node( string& seq, Node* flip, int graph )
: Node( seq )
{
    drxn_ = graph;
    ends_[0] = 0;
    ends_[1] = seq_.size();
    for ( auto& read : flip->reads_ )
    {
        int32_t coords[2]{ flip->ends_[1] - read.second[1], flip->ends_[1] - read.second[0] };
        add( read.first, coords[0], coords[1], read.second.redundant, read.second.ignore, -read.second.coords[2] );
    }
    setCoverage();
}
...
: Node( read.seq )
{
    ...
    add( read.id, read.coords[0], read.coords[1], false );
    readTest();
    setOrigin();
}
...
{
    for ( Edge& e : edges_[drxn] ) if ( !e.leap ) assert( ( drxn ? this : e.node )->getSeqEnd( e.ol, 1 ) == ( drxn ? e.node : this )->getSeqEnd( e.ol, 0 ) );
...
void Node::addEdge( Node* node, int overlap, bool drxn, bool doOffset, bool isLeap )
{
    ...
    addEdge( Edge( node, overlap, isLeap || overlap <= 0 ), drxn, true );
    edgeTest( drxn );
    node->edgeTest( !drxn );
...
void Node::cleanEdges( NodeRoll& nodes, bool drxn )
{
    
    vector<Edge> edges = edges_[drxn];
    for ( Edge& e : edges ) e.node->removeEdge( this, !drxn, true );
    for ( Edge& e : edges ) if ( e.node->updateBad() )
    {
        Nodes dependent( e.node, drxn, true );
        bool cull = true;
        for ( Node* node : dependent.nodes ) for ( Edge& e : node->edges_[!drxn] ) if ( !dependent.find( e.node ) ) cull = false;;
        if ( cull ) for ( Node* node : dependent.nodes ) nodes.erase( node );
    }
}

void Node::clearEdges( bool drxn, bool stop )
{
    for ( Edge &e: edges_[drxn] ) e.node->removeEdge( this, !drxn, false );
    if ( !stop ) for ( Edge &e: edges_[drxn] ) e.node->stop_[!drxn] = 0;
    edges_[drxn].clear();
}
...
vector<Node*> Node::getPath()
{
    ...
    for ( int d : { 0, 1 } ) getPath( path, d );
    return path;
}

void Node::getPath( vector<Node*>& path, bool drxn )
{
    Node* node = edges_[drxn].size() == 1 && edges_[drxn][0].node->edges_[!drxn].size() == 1 ? edges_[drxn][0].node : NULL;
    if ( node ) path.insert( drxn ? path.end() : path.begin(), node );
    if ( node && count( path.begin(), path.end(), node ) == 1 ) node->getPath( path, drxn );
}
...
string Node::getSeq( vector<Node*>& path )
{
    ...
    {
        Edge e = path[i]->getEdge( path[i-1], 0 );
        if ( e.ol < 0 ) seq += string( -e.ol, 'N' );
        seq += path[i]->seq_.substr( max( 0, e.ol ) );
    }
    return seq;
}
...
{
    return node && ( node == this || ( cloned_ && cloned_->find( node ) ) );
}
...
bool Node::isContinue( int readLimit, bool drxn )
{
    readLimit -= countReads( true );
    if ( readLimit < 0 ) return false;
    for ( Edge& e : edges_[drxn] ) if ( e.node->isContinue( readLimit ), drxn ) return true;
    return isContinue( drxn );
}

bool Node::isContinue( int readLimit, int ol, int minOl, bool drxn )
{
    if ( ol < minOl ) return false;
    readLimit -= countReads( true );
    if ( readLimit < 0 ) return false;
    for ( Edge& e : edges_[drxn] ) if ( e.node->isContinue( readLimit, ol - size() + e.ol, minOl, drxn ) ) return true;
    return isContinue( drxn );
}
