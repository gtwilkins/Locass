...
void MappedSeqs::cull( vector<MappedSeqs>& alts )
{
    for ( int i = 0; i < reads.size(); i++ ) if ( reads[i].coords[0] != reads[i].tether[0] || reads[i].coords[1] != reads[i].tether[1] )
    {
        bool bad = false;
        for ( MappedSeqs& ms : alts ) if ( &ms != this && ms.usedIds.find( reads[i].id ) != ms.usedIds.end() )
        {
            for ( ReadStruct& read : ms.reads ) if ( read.id == reads[i].id )
            {
                if ( bad = ( read.tether[1] - read.tether[0] > reads[i].tether[1] - reads[i].tether[0] ) ) break;
            }
            if ( bad ) break;
        }
        if ( bad ) reads.erase( reads.begin() + i-- );
    }
}
