...
bool QueryNode::isIgnore( int len, vector<QueryNode*> &exts, bool drxn )
{
    for ( QueryNode* alt : exts ) if ( alt != this )
    {
        bool ignore = true;
        assert( len < min( alt->ext.size(), ext.size() ) );
        for ( int i = 0; ignore && i < len; i++ ) ignore = drxn ? ext[i] == alt->ext[i] : ext.end()[-i-1] == alt->ext.end()[-i-1];
        assert( ignore );
